...
int main(int argc, char* argv[])
{
	...
	{
		Mat temImage = imread("template" + to_string(i) + ".png");
	...
	Rect rect;
	//rect = detectCard(image, scene_corners[index]);
	//image = normalizeRotation(image, scene_corners[index]);
	//scene_corners = detectCard(detecter, temImage, image, scene_corners, &rect);
	//image = image(rect);
	//Mat result = doocr.NormlizImage(image, scene_corners[0], scene_corners[1], scene_corners[2], scene_corners[3]);
	//imshow("image",image);
